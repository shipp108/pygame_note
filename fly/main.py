...
class Plane():
    ...
    def draw(self, screen):
        screen.blit(self.plane_image, (self.x, self.y, self.width,
                                       self.height))
    ...
    def collision(self, eneme):
        if not (self.x > eneme.x + eneme.width or self.x + self.width < eneme.x
...
class Enemy():
    def __init__(self, enemy_image):
        ...
        self.x = random.choice(
            range(0, int(SCREEN_SIZE[0] - self.width / 2), 71))
        self.y = 0
    ...
    def draw(self, screen):
        screen.blit(self.enemy_image, (self.x, self.y, self.width,
                                       self.height))
...
class Game():
    def __init__(self):
        ...
        pygame.display.set_caption('AI plane')

        self.ai = neuro_evolution.NeuroEvolution()
        self.generation = 0

        self.max_enemes = 1
        # load plane enemy image
    ...
    def update(self, screen):
        ...
        if self.generation * 500 < self.score:
            self.start()

        print(
            "alive:{}, generation:{}, score:{}".format(
                self.alives, self.generation, self.score))
