...
class Neuron():
    ...
    def __repr__(self):
        return 'Neuron weight size:{}  biase value:{}'.format(
            len(self.weights), self.biase)
...
class Layer():
    ...
    def __repr__(self):
        return 'Layer ID:{}  Layer neuron size:{}'.format(
            self.index, len(self.neurons))
        ...
        for i in data['network']:
            ...
            for j in range(len(layer.neurons)):
                for k in range(len(layer.neurons[j].weights)):
                    layer.neurons[j].weights[k] = data['weights'][
                        index_weights]
                    index_weights += 1
            previous_neurons = i
            index += 1
        ...
        for i in range(len(self.layers)):
            ...
            for j in range(len(self.layers[i].neurons)):
                ...
                for k in range(len(prev_layer.neurons)):
                    sum += prev_layer.neurons[k].biase * self.layers[
...
# 杂交+突变
    ...
    def breed(self, genome1, genome2, n_child):
        ...
        for n in range(n_child):
            #data = genome1
            data = Genome(0, {'weights':genome1.network_weights['weights'][:], 'network':genome1.network_weights['network'][:]})
            for i in range(len(genome2.network_weights['weights'])):
                if random.random() <= 0.5:
                    data.network_weights['weights'][
                        i] = genome2.network_weights['weights'][i]
    ...
    def generate_next_generation(self):
        ...
        while True:
            for i in range(max_n):
                childs = self.breed(self.genomes[i], self.genomes[max_n],
                                    n_child if n_child > 0 else 1)
    ...
    def next_generation(self):
        ...
        if low_historic:
            if len(self.generations.generations) >= 2:
                genomes = self.generations.generations[
                    len(self.generations.generations) - 2].genomes
        ...
        if historic != -1:
            if len(self.generations.generations) > historic + 1:
                del self.generations.generations[
                    0:len(self.generations.generations) - (historic + 1)]
